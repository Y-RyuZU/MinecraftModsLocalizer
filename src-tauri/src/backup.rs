...
pub fn create_backup(
    ...
    for file_path in &file_paths {
        ...
        if source_path.exists() {
            ...
            if let Err(e) = fs::copy(source_path, &dest_path) {
                logger.warning(&format!("Failed to backup file {}: {}", file_path, e), Some("BACKUP"));
            } else {
                backed_up_files.push(dest_path.to_string_lossy().to_string());
                logger.debug(&format!("Backed up file: {} -> {}", file_path, dest_path.display()), Some("BACKUP"));
            }
        } else {
            logger.warning(&format!("Source file not found for backup: {}", file_path), Some("BACKUP"));
        }
    }
    ...
    logger.info(&format!("Backup created successfully: {} ({} files)", backup_path, backed_up_files.len()), Some("BACKUP"));
...
pub fn list_backups(
    ...
    for session_entry in session_dirs {
        let session_entry = session_entry
            .map_err(|e| format!("Failed to read session directory entry: {}", e))?;
        ...
        for backup_entry in backup_entries {
            ...
            let can_restore = original_files_dir.exists() && 
                original_files_dir.read_dir().map(|mut entries| entries.next().is_some()).unwrap_or(false);
...
pub fn restore_backup(
    ...
    logger.info(&format!("Restoring backup: {} to {}", backup_id, target_path), Some("BACKUP"));
    ...
    for backup_file in backup_files {
        let backup_file = backup_file
            .map_err(|e| format!("Failed to read backup file entry: {}", e))?;
        ...
        if let Err(e) = fs::copy(&source_path, &dest_path) {
            logger.warning(&format!("Failed to restore file {}: {}", source_path.display(), e), Some("BACKUP"));
        } else {
            restored_count += 1;
            logger.debug(&format!("Restored file: {} -> {}", source_path.display(), dest_path.display()), Some("BACKUP"));
        }
    }
    
    logger.info(&format!("Backup restoration completed: {} files restored", restored_count), Some("BACKUP"));
    Ok(())
}
...
pub fn delete_backup(
    ...
    if backup_path.exists() {
        ...
        logger.info(&format!("Backup deleted successfully: {}", backup_id), Some("BACKUP"));
    } else {
        logger.warning(&format!("Backup directory not found for deletion: {}", backup_id), Some("BACKUP"));
    }
...
pub fn prune_old_backups(
    ...
    logger.info(&format!("Pruning backups older than {} days", retention_days), Some("BACKUP"));
        ...
        if let Ok(backup_time) = chrono::DateTime::parse_from_rfc3339(&backup.metadata.timestamp) {
            if backup_time.with_timezone(&chrono::Utc) < cutoff_time {
                if let Ok(()) = delete_backup(backup.metadata.id.clone(), logger.clone()) {
                    deleted_count += 1;
                    logger.debug(&format!("Pruned old backup: {}", backup.metadata.id), Some("BACKUP"));
                }
            }
        }
    }
    
    logger.info(&format!("Backup pruning completed: {} backups removed", deleted_count), Some("BACKUP"));
    Ok(deleted_count)
}
...
#[tauri::command]
pub fn get_backup_storage_size(
    logger: State<Arc<AppLogger>>,
) -> Result<u64, String> {
    let logs_dir = PathBuf::from("logs").join("localizer");
    ...
    logger.debug(&format!("Total backup storage size: {} bytes", total_size), Some("BACKUP"));
    Ok(total_size)
}