...
describe('Progress Tracking E2E Tests', () => {
  ...
  test('should update progress for each chunk completion', async () => {
    ...
    const mockIncrementChunks = mock(() => {
      chunkProgressUpdates++;
    });
    
    const mockIncrementMods = mock(() => {
      modProgressUpdates++;
    });
  ...
  // Use Bun's timeout syntax
  test('should handle progress tracking with multiple mods', async () => {
  ...
  test('should simulate slow translation to verify real-time progress', async () => {
    // Create content
  ...
  test('should handle missing incrementCompletedChunks gracefully', async () => {
    const content = {
    ...
    // Run without incrementCompletedChunks - should not throw
    let didThrow = false;
    try {
      await runTranslationJobs({
        jobs: [job],
